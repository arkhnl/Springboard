...
SELECT name,
       ...
       CASE WHEN monthlymaintenance < 100 THEN 'cheap'
	    ELSE 'expensive' END AS maintainenace_label
   FROM country_club.Facilities
...
SELECT CASE WHEN bookings.memid = '0' THEN bookings.slots*facilities.guestcost
...
SELECT sub.*
	FROM(SELECT CASE WHEN bookings.memid = '0' THEN bookings.slots*facilities.guestcost
			    ELSE bookings.slots*facilities.membercost END AS total_cost,
		 		 CONCAT(members.firstname,' ',members.surname) AS member_name,
		 		 members.memid,
		 		 bookings.bookid,
...
SELECT sub.total_revenue,
	   sub.name,
	   SUM(sub.total_revenue) OVER (PARTITION BY sub.name) AS facility_revenue
		FROM(SELECT CASE WHEN bookings.memid = '0' THEN bookings.slots*facilities.guestcost
